...
namespace CleanArchitectureNetCore.Application.Features.Streamers.Commands.CreateStreamer
{
    public class CreateStreamerCommandHandler : IRequestHandler<CreateStreamerCommand, int>
    {
        ...
        public async Task<int> Handle(CreateStreamerCommand request, CancellationToken cancellationToken)
        {
            ...
            await SendEmail(newStreamer, new List<string> { "luisfonseca9521@gmail.com" });
            return newStreamer.Id;
        }

        private async Task SendEmail(Streamer streamer, List<string> emails)
        {
            List<string> to = new List<string>(emails.ToList());
